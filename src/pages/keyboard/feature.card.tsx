...
export default function FeatureCards() {
    ...
    const features = data.map((item) => (
        ...
        >
            <Card withBorder shadow="md" radius="md" className={classes.card} padding="xl">
                <item.icon style={{
                    width: rem(50),
                    height: rem(50),
                }}
                ...
                />
                    <Title fz="lg" fw={500} className={classes.cardTitle} mt="md">
                        {item.title}
                    </Title>

                    {item.text.map((item, index) => (
                        <Text fz="sm" c="dimmed" mt="sm" key={index}>{item}</Text>
                    ))}
            </Card>
        </motion.div>
    ))
