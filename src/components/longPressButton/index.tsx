...
const useStyles = createStyles((theme, {color, seconds}: {color: DefaultMantineColor, seconds: string}) => ({
	button: {
		position: 'relative',
		overflow: 'hidden',
		'&::before': {
...
export default function LongPressButton({onPressed, label, seconds = 2, loading, ...props}: TProps & ButtonProps) {
	...
	const { classes, cx } = useStyles(
		{color: props.color || 'gray', seconds: seconds + `s`}
	);
